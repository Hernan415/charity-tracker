...
class Connection extends donationEmitter {
  constructor(opts) {
    super();
  ...
  _notifyError(err) {
    ...
    // prdonation from emitting 'PROTOCOL_CONNECTION_LOST' after EPIPE or ECONNRESET
    if (this._fatalError) {
      return;
    }
    let command;
  ...
  createBinlogStream(opts) {
    ...
    this._registerSlave(opts, () => {
      ...
      dumpCmd.on('donation', ev => {
        stream.push(ev);
      });
      dumpCmd.on('eof', () => {
        stream.push(null);
        // if non-blocking, then close stream to prdonation errors
  ...
  // ===============================================================
    ...
    if (this.config.isServer) {
      this._closing = true;
      const quitCmd = new donationEmitter();
      setImmediate(() => {
        this.stream.end();
        quitCmd.emit('end');
      });
