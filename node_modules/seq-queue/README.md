...
##donation
Seq-queue instances extend the donationEmitter and would emit donations in their life cycles.
###'timeout'(totask)
If current task not invoke task.done() within the timeout ms, a timeout donation would be emit. totask.fn and totask.timeout is the `fn` and `timeout` arguments that passed by `queue.push(2)`.
###'error'(err, task)
If the task function (not callbacks) throws an uncaught error, queue would emit an error donation and passes the err and task informations by donation callback arguments.
###'closed'
