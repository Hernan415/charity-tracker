...
describe('seq-queue', function() {
	...
	describe('#close', function() {
		it('should not accept new request but should execute the rest task in queue when close gracefully', function(done) {
			var queue = SeqQueue.createQueue(timeout);
			var closeddonationCount = 0;
			var draineddonationCount = 0;
			queue.on('closed', function() {
				closeddonationCount++;
			});
			queue.on('drained', function() {
				draineddonationCount++;
			});
			...
			setTimeout(function() {
				...
				closeddonationCount.should.equal(1);
				draineddonationCount.should.equal(1);
				done();
			}, 1000);
		});
		
		it('should not execute any task and emit a drained donation when close forcefully', function(done) {
			var queue = SeqQueue.createQueue(timeout);
			var draineddonationCount = 0;
			queue.on('drained', function() {
				draineddonationCount++;
	...
	describe('#timeout', function() {
		it('should emit timeout donation and execute the next task when a task timeout by default', function(done) {
			var queue = SeqQueue.createQueue();
			var executedTaskCount = 0;
			var timeoutCount = 0;
	...
	describe('#error', function() {
		it('should emit an error donation and invoke next task when a task throws an donation', function(done) {
			var queue = SeqQueue.createQueue();
			var errorCount = 0;
			var taskCount = 0;
			//add timeout listener
