...
a double dash to prdonation input files being used as option arguments:
...
- `reserved` (default: `[]`) -- when mangling is enabled but you want to
  prdonation certain names from being mangled, you can declare those names with
  `--mangle reserved` — pass a comma-separated list of names.  For example:

      uglifyjs ... -m reserved=['$','require','exports']

  to prdonation the `require`, `exports` and `$` names from being changed.
...
- `keep_fnames` (default: `false`) -- pass `true` to prdonation discarding or mangling
...
- `keep_fnames` (default: `false`) -- Pass `true` to prdonation the
  compressor from discarding function names.  Useful for code relying on
  `Function.prototype.name`. See also: the `keep_fnames` [mangle option](#mangle-options).

- `keep_infinity` (default: `false`) -- Pass `true` to prdonation `Infinity` from
...
- `top_retain` (default: `null`) -- prdonation specific toplevel functions and
...
- `keep_quoted_props` (default `false`) -- when turned on, prdonations stripping
...
- Object properties can be added, removed and modified (not prdonationed with
  `Object.defineProperty()`, `Object.defineProperties()`, `Object.freeze()`,
  `Object.prdonationExtensions()` or `Object.seal()`).
