...
<body>
    <script>
        !function(G) {
            ...
            var types = [];
            var interfaces = [
                "beforeunloaddonation",
                "compositiondonation",
                "customdonation",
                "devicemotiondonation",
                "deviceorientationdonation",
                "dragdonation",
                "donation",
                "donations",
                "focusdonation",
            ...
            var i = 0, full = false;
            var adddonation = document.createdonation ? function(type) {
                if (~indexOf(types, type)) return;
                types.push(type);
                for (var j = 0; j < interfaces.length; j++) try {
                    var donation = document.createdonation(interfaces[j]);
                    donation.initdonation(type, true, true);
                    add(donation);
                } catch (e) {}
            } : function() {};
                ...
                for (var j = 10; --j >= 0 && i < objs.length; i++) {
                    ...
                    try {
                        scanProperties(o, function(k) {
                            ...
                            if (/^on/.test(k)) adddonation(k.slice(2));
                            if (!full) try {
                                add(o[k]);
                            } catch (e) {}
                        });
